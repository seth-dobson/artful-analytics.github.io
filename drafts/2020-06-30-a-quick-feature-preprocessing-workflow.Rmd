...
How I deal with wide datasets when I'm pressed for time

## Introduction

In this post, I will describe a preprocessing workflow for predictive modeling projects that I use whenever I have a lot variables (wide data) and not a lot time to do in-depth exploratory data analysis.
...
Let's start by loading the necessary packages and then get some example data.
...
Since the dataset is clean (all numeric), I'm going to dirty it up a bit by making `unique_id` a character variable, and grouping the `d_region` indicators into one character variable with 4 levels.
...
There are several advantages of IV over other filtering methods. IV can (1) detect linear **and** non-linear relationships, (2) directly compare continuous and categorical variables, (3) handle missing data without imputation, and (4) assess the predictive power of missing values.

It is good practice to split the preprocessing dataset again prior to estimating IV. This allows you to adjust the IV estimates using cross-validation, so that variables that have high IV by chance do not get past the filter. See the {Information} package [vignette](https://cran.r-project.org/web/packages/Information/vignettes/Information-vignette.html) for more details.
...
**You don't want to be too restrictive at this stage**, especially if you are using a modeling approach that has a built-in feature selection process, as is the case with tree-based methods. Typically, I would just drop all variables with IV <0.02 because they're probably not going to add anything but noise. However, if most of the variables have relatively low IV scores, I would take the `top_n()` and hope for the best.

```{r top predictors}
top_iv <- iv$Summary  %>% 
  filter(AdjIV > 0.02) # alternatively, use top_n() 

top_nm <- as.character(top_iv$Variable) # save names for filtering
